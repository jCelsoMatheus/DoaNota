...
window.WebAPI = {
	buildException: function (ex) {
		if (ex.message) {
			return { success: false, status: -1, value: "Ocorreu o seguinte erro: " + ex.message, exceptionType: (ex.name || "Error") };
		} else {
			return { success: false, status: -1, value: "Ocorreu o seguinte erro: " + ex, exceptionType: (ex.name || "Error") };
		}
	},
		...
		if (req.status === 200) {
			...
			try {
				...
				if (ex.name === "SyntaxError") {
					return { success: false, status: req.status, value: req.responseText, exceptionType: "System.Exception" };
				}
